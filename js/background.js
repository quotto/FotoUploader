var changeUploadingModal = function(tab_id,status_text,message) {
  "use strict";
  var send_message = '{"status": "' + status_text + '","message": "' + message + '"}'; 
    ...
    chrome.storage.local.get(['FotolifeUploader_HatenaId','FotolifeUploader_ApiKey','FotolifeUploader_Folder'],function(value){
      ...
      xhr.onload = function(e) {
        ...
        if(descriptor_hex.substr(0,4) === 'ffd8') {
          ...
          changeUploadModal(tab.id,'error','この画像はアップロードできません。');
          return;
        }
        ...
        $.ajax({
          url: upload_url,
          method: 'POST',
          data: {descriptor: descriptor, imagedata: btoa(binaryData), folder: folder_name,id: hatena_id,api_key: api_key, digest: digest, nonce: base64nonce, timestamp: timestamp},
          dataType: 'json'
